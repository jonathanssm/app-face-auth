...
export class AppHttpUtil {
    ...
    */
    public static criarHttpRequest(arquivo: Array<File>, url: string, dadosExtras?: object, tipoResposta: TipoResposta = TipoResposta.JSON): HttpRequest<FormData> {
        const corpoRequisicao: FormData = new FormData()
        arquivo.forEach((arquivo: File, index: number) => corpoRequisicao.append('arquivo', arquivo, arquivo.name))
